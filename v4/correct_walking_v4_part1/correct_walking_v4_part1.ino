...
int lAngle = max_angle; // left angle
int rAngle = max_angle; // forward angle (right)
...
void loop() {
  ...
  if (angleDifference < 0) {
    ...
    Serial.println("");
    frAngle = max_angle + .5*angleDifference;
    brAngle = -(max_angle+.5*angleDifference);
    flAngle = max_angle;
    blAngle = -max_angle;
  }
