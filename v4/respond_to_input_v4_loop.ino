...
int rotateLeg(Servo servo, int desiredAngle) {
  if (servo.read() >= desiredAngle){
    return 1;
  }
  else if ((servo.read() + 5) > desiredAngle){ // if within 5, go to final angle
...
void rotateAll1() {
  int set = 0;
  while (set < 4) { // while not all set
    set += rotateLeg(rotate1, to1(flAngle));
    set += rotateLeg(rotate2, to2(brAngle));
    set += rotateLeg(rotate3, to3(blAngle));
    set += rotateLeg(rotate4, to4(frAngle));
  }
  delay(1000);
}
...
void rotateAll2() {
  int set = 0;
  while (set < 4) {
    set += rotateLeg(rotate1, to1(blAngle));
    set += rotateLeg(rotate2, to2(frAngle));
    set += rotateLeg(rotate3, to3(flAngle));
    set += rotateLeg(rotate4, to4(brAngle));
  }
  delay(1000);
}
